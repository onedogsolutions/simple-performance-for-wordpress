...
* Fixed: A cached page generated while logged in could carry no Content-Security-Policy and then be served to logged-out visitors for the rest of the cache TTL, silently switching the policy off for the people it protects. Front-end pages viewed while logged in are now marked uncacheable whenever "Do not apply to logged-in users" is on.
* Fixed: A violation-collection window that expired on its own left cached pages still advertising report-uri, so browsers kept POSTing to an endpoint that had closed and was answering 403. The deadline now schedules a real event that closes the window and purges the page cache.
